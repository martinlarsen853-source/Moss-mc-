...
export async function GET(req: Request) {
  ...
  const { data: metrics } = await supabase.from('daily_metrics').select('*').gte('date', past14).order('date', { ascending: false });
  const { data: recentWorkouts } = await supabase.from('workouts').select('date,type,name,avg_pace_sec_per_km,avg_hr,drag_median_pace_sec,ai_analysis').order('date', { ascending: false }).limit(10);
  ...
  const coachContext = {
    type: 'ukessjekk',
    question: `Det er ${dayOfWeek === 4 ? 'torsdag' : 'fredag'}. Martin har Hyrox lørdag kl 10:30 (hard økt, RPE 8). Bør han kjøre Hyrox som planlagt, ta det ned til RPE 6-7, eller bytte til et annet format? Forklar med tall-begrunnelse og hva valget gir på kort og lang sikt. Maks 6 linjer.`,
    todayMetrics: {
    ...
    trainingLoad: { ctl: todayMetrics?.ctl ?? undefined, atl: todayMetrics?.atl ?? undefined, tsb: todayMetrics?.tsb ?? undefined },
    hrvTrend,
  ...
  await supabase.from('coaching_messages').insert({
    type: 'weekly-check', message: analysis,
  ...
  if (subs?.length) {
    ...
    await sendPushToAll(subs, {
      ...
      body: analysis.split('\n')[0].substring(0, 130),
      tag: 'weekly-check', url: '/coaching', requireInteraction: true,
    });
  }
