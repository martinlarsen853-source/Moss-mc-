...
export async function GET(req: Request) {
  ...
  const { data: recentWorkouts } = await supabase.from('workouts').select('date,type,name,avg_pace_sec_per_km,avg_hr,drag_median_pace_sec,ai_analysis').order('date', { ascending: false }).limit(7);
  ...
  await supabase.from('coaching_messages').insert({
    type: 'pre-workout', message: briefing,
  ...
  if (subs?.length) {
    await sendPushToAll(subs, {
      ...
      body: briefing.split('\n')[0].substring(0, 120),
      tag: 'pre-workout', url: '/coaching', requireInteraction: true,
    });
  }
