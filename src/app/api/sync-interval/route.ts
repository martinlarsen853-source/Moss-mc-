...
export async function GET(req: Request) {
  ...
  const synced = { activities: 0, wellness: 0, errors: [] as string[] };
  ...
  try {
    ...
    for (const act of activities) {
      const existing = await supabase.from('workouts').select('id').eq('interval_id', act.id as string).single();
      ...
      const workoutRow = {
        ...
        hr_zone_1_min: null, hr_zone_2_min: null, hr_zone_3_min: null, hr_zone_4_min: null, hr_zone_5_min: null,
        drag_median_pace_sec: null as number | null,
      };
      ...
      if (workoutRow.type === 'Run' && act.streams) {
        ...
        const { data: workout } = await supabase.from('workouts').insert(workoutRow).select().single();
