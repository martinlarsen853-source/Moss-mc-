...
// POST /api/post-workout  – called after sync detects a new workout
export async function POST(req: Request) {
  ...
  // Get drag intervals
  const { data: drags } = await supabase
    .from('drag_intervals')
    .select('*')
  ...
  const { data: recentWorkouts } = await supabase
    .from('workouts')
    .select('date,type,name,avg_pace_sec_per_km,avg_hr,drag_median_pace_sec')
    .order('date', { ascending: false })
    .limit(7);
  ...
  const coachContext = {
    ...
    trainingLoad: {
      ctl: today?.ctl ?? undefined,
      atl: today?.atl ?? undefined,
      tsb: today?.tsb ?? undefined,
    },
    hrvTrend,
  ...
  if (subs?.length) {
    ...
    await sendPushToAll(subs, {
      title: `Økt-analyse: ${workout.name}`,
      body: shortMsg,
      tag: 'post-workout',
      url: `/workouts`,
    });
