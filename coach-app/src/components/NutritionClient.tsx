    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-24">
      ...
      <div className="mx-4 mb-3 rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
        ...
        <div className="h-3 bg-zinc-800 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${todayProtein >= proteinGoal ? 'bg-emerald-500' : 'bg-amber-500'}`}
      ...
      <div className="mx-4 mb-3">
        ...
        <button
          ...
          className="w-full rounded-2xl bg-zinc-900 border border-dashed border-zinc-700 p-4 flex items-center gap-3 active:bg-zinc-800"
        >
      ...
      <div className="px-4 mb-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {presets.map((p) => (
            <button
              key={p.id}
              onClick={() => logPreset(p.id)}
              disabled={logging === p.id}
              className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3 text-left active:bg-zinc-800 disabled:opacity-50"
            >
              ...
              return (
                <div key={date} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full bg-zinc-800 rounded-sm relative" style={{ height: `${pct}%`, minHeight: 4 }}>
                    <div
                      className={`absolute bottom-0 left-0 right-0 rounded-sm ${val.protein >= proteinGoal ? 'bg-emerald-500' : isToday ? 'bg-amber-400' : 'bg-zinc-600'}`}
                      style={{ height: '100%' }}
                    />
                  </div>
                  <span className="text-[9px] text-zinc-600">
                    {new Date(date).toLocaleDateString('no', { weekday: 'short' }).charAt(0)}
                  </span>
                </div>
              );
            })}
              ...
              <div key={l.id} className="flex justify-between items-center py-2.5 border-b border-zinc-900">
                <div>
                  ...
                  <p className="text-xs text-zinc-600">
                    {new Date(l.time).toLocaleTimeString('no', { hour: '2-digit', minute: '2-digit' })} · {l.source === 'photo' ? '📷' : l.source === 'preset' ? '⚡' : '✏️'}
                  </p>
                </div>
                <div className="text-right">
