    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-24">
      ...
      <div className={`mx-4 mb-3 rounded-2xl border p-4 ${formColors.bg} ${formColors.border}`}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className={`text-sm font-medium ${formColors.text}`}>
            Form: {formStatus.charAt(0).toUpperCase() + formStatus.slice(1)}
          </span>
        </div>
      ...
      {latestCoachMessage && (
        <div className="mx-4 mb-3 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
          ...
          <p className="text-sm leading-relaxed text-zinc-200 whitespace-pre-line line-clamp-4">
            {latestCoachMessage}
          </p>
        </div>
      )}
      ...
      <div className="mx-4 mb-3 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <p className="text-sm font-semibold">
            <span className={proteinToday >= proteinGoal * 0.9 ? 'text-emerald-400' : 'text-white'}>
              {proteinToday}g
            </span>
        ...
        <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${proteinToday >= proteinGoal ? 'bg-emerald-500' : 'bg-amber-500'}`}
      ...
      <div className="mx-4 mb-3">
        ...
        <button
          ...
          className="w-full rounded-2xl border border-dashed border-zinc-700 bg-zinc-900/40 p-4 flex items-center gap-3 active:bg-zinc-800 transition-colors"
        >
        ...
        <div className="px-4 mb-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {presets.map((p) => (
              <button
                key={p.id}
                onClick={() => logPreset(p.id, p.name)}
                disabled={logging === p.id}
                className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3 text-left active:bg-zinc-800 disabled:opacity-60 transition-colors"
              >
