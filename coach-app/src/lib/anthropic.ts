...
export async function analyzeFood(imageBase64: string, mimeType: string): Promise<{
  description: string;
  kcal: number;
  protein_g: number;
  carbs_g: number;
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: 'text',
            text: `Analyser dette matbildet. Gi estimat på kalorier og makros.
...
function buildUserMessage(ctx: CoachContext): string {
  ...
  if (ctx.hrvTrend?.length) {
    lines.push(`HRV-trend siste ${ctx.hrvTrend.length} dager: ${ctx.hrvTrend.join(', ')}`);
  }
