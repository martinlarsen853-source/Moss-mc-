// Interval.icu API client
// Docs: https://forum.intervals.icu/t/api/87
...
export async function getActivities(oldest: string, newest: string) {
  const res = await fetch(
    `${BASE}/athlete/${athleteId()}/activities?oldest=${oldest}&newest=${newest}`,
    { headers: headers() }
  );
...
export async function getWellness(startDate: string, endDate: string) {
  const res = await fetch(
    `${BASE}/athlete/${athleteId()}/wellness?oldest=${startDate}&newest=${endDate}`,
    { headers: headers() }
  );
...
export async function getWellnessDay(date: string) {
  const res = await fetch(`${BASE}/athlete/${athleteId()}/wellness/${date}`, {
    headers: headers(),
  });
...
export async function getTrainingLoad() {
  ...
  // Return last entry with CTL/ATL/TSB
...
// Extract drag intervals from an activity's power/HR/pace streams
// A drag = consecutive segment ≥3 min with avg HR 167–173
export function extractDragIntervals(streams: {
  time: number[];
  heartrate: number[];
  speed: number[];
}) {
  ...
  for (let i = 0; i < streams.time.length; i++) {
    ...
    if (inZone && start === null) {
      ...
      if (duration >= MIN_DURATION) {
        const segment = {
          hrs: streams.heartrate.slice(start, i),
          speeds: streams.speed.slice(start, i),
        };
        const avgHr = segment.hrs.reduce((a, b) => a + b, 0) / segment.hrs.length;
        const avgSpeed = segment.speeds.reduce((a, b) => a + b, 0) / segment.speeds.length; // m/s
        const avgPace = avgSpeed > 0 ? 1000 / avgSpeed : 0;
        drags.push({
          duration_sec: duration,
          avg_pace_sec_per_km: avgPace,
          avg_hr: Math.round(avgHr),
          max_hr: Math.max(...segment.hrs),
        });
      }
      start = null;
    }
...
export function dragMedian(drags: Array<{ avg_pace_sec_per_km: number }>): number | null {
  ...
  return sorted.length % 2 !== 0
    ? sorted[mid].avg_pace_sec_per_km
    : (sorted[mid - 1].avg_pace_sec_per_km + sorted[mid].avg_pace_sec_per_km) / 2;
}
